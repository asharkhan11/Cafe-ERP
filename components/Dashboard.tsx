...
const Dashboard: React.FC<DashboardProps> = ({ orders, products, config, onNavigate }) => {
  ...
  const salesData = orders.slice(-7).map(o => ({
    ...
    <div className="space-y-6 pb-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
        ...
        >
          ...
          <p className="text-xl lg:text-3xl font-black text-slate-900 mt-1">{config.currency}{totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
          <div className="mt-2 text-[10px] text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-full w-fit">↑ 12%</div>
        </button>
        ...
        >
          ...
          <p className="text-xl lg:text-3xl font-black text-slate-900 mt-1">{config.currency}{totalOrders > 0 ? (totalRevenue / totalOrders).toFixed(2) : '0.00'}</p>
